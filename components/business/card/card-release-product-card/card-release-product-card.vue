		...
		<block v-else>
			<view class="card-cover">
				...
				<view class="stock-badge" v-if="item.stock">
					<image class="badge-icon stock-pop" :src="getIconByType(item.type)" mode="aspectFit" />
					<text class="badge-text stock-pop" :key="item.stock">{{ item.stock }}</text>
				</view>
			</view>
			<view class="card-info">
				...
				<view class="sales-row" v-if="item.salesNum">
					...
					<text class="sales-text">đã bán trong 3 giờ gần đây</text>
				</view>
