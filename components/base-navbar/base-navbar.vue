<template>
    <view class="base-navbar">
        <view
            ...
            class="base-navbar__placeholder"
            :style="placeholderStyle"
        />

        <view
            class="base-navbar__wrap"
            :class="wrapClasses"
            :style="[wrapStyleObject, customStyle]"
        >
            ...
            <view
                v-if="safeAreaTop"
                class="base-navbar__status-bar"
                :style="statusBarStyle"
            />

            <view
                class="base-navbar__main"
                :class="mainClasses"
                :style="[mainStyleObject, contentStyle]"
            >
                <view
                    class="base-navbar__side base-navbar__side--left"
                    :style="[leftSideStyleObject, leftStyle]"
                    @tap="handleLeftClick"
                >
                    <slot name="left">
                        <view v-if="hasDefaultLeft" class="base-navbar__action">
                            <view
                                v-if="showBackArrow"
                                class="base-navbar__back-icon"
                                :style="backIconStyle"
                            />
                            <text
                                v-else-if="leftIcon"
                                class="base-navbar__icon-text"
                            ...
                            <text
                                v-if="leftLabel"
                                class="base-navbar__action-text"
                                :style="leftTextStyleObject"
                            >
                                {{ leftLabel }}
                            </text>
                        </view>
                ...
                <view
                    v-if="showCenterArea"
                    class="base-navbar__center"
                    :class="{ 'base-navbar__center--left': titleAlign === 'left' }"
                    :style="centerStyleObject"
                    @tap="handleTitleClick"
                >
                    <slot name="title">
                        <view
                            class="base-navbar__title-group"
                            :class="{ 'base-navbar__title-group--left': titleAlign === 'left' }"
                        >
                            <text
                                v-if="title"
                                class="base-navbar__title"
                                :style="[titleStyleObject, titleStyle]"
                            >
                                {{ title }}
                            </text>
                            <text
                                v-if="subtitle"
                                class="base-navbar__subtitle"
                ...
                <view
                    class="base-navbar__side base-navbar__side--right"
                    :style="[rightSideStyleObject, rightStyle]"
            ...
            <view v-if="hasBottomSlot" class="base-navbar__bottom">
                <view
                    class="base-navbar__bottom-inner"
                    :style="[bottomStyleObject, bottomStyle]"
                >
                    <slot name="bottom" />
                </view>
            </view>
...
.base-navbar { width: 100%; }
.base-navbar__placeholder { width: 100%; }
.base-navbar__wrap { position: relative; width: 100%; box-sizing: border-box; }
.base-navbar__wrap--fixed { position: fixed; }
.base-navbar__background, .base-navbar__background-slot { position: absolute; top: 0; right: 0; bottom: 0; left: 0; }
.base-navbar__background-slot { overflow: hidden; }
.base-navbar__status-bar, .base-navbar__main, .base-navbar__bottom { position: relative; z-index: 1; }
.base-navbar__main { display: flex; align-items: center; width: 100%; box-sizing: border-box; }
.base-navbar__main--no-center .base-navbar__side--right { margin-left: auto; }
.base-navbar__side { height: 100%; display: flex; align-items: center; overflow: hidden; flex-shrink: 0; }
.base-navbar__side--left { justify-content: flex-start; }
.base-navbar__side--right { justify-content: flex-end; }
.base-navbar__center { flex: 1; min-width: 0; height: 100%; display: flex; align-items: center; justify-content: center; box-sizing: border-box; overflow: hidden; }
.base-navbar__center--left { justify-content: flex-start; }
.base-navbar__title-group { min-width: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; }
.base-navbar__title-group--left { align-items: flex-start; }
.base-navbar__title, .base-navbar__subtitle, .base-navbar__action-text, .base-navbar__icon-text { max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.base-navbar__title { line-height: 1.25; }
.base-navbar__subtitle { line-height: 1.2; margin-top: 4px; }
.base-navbar__action { min-width: 0; display: inline-flex; align-items: center; }
.base-navbar__action--right { justify-content: flex-end; }
.base-navbar__back-icon { width: 10px; height: 10px; border-left-width: 2px; border-left-style: solid; border-bottom-width: 2px; border-bottom-style: solid; box-sizing: border-box; transform: rotate(45deg); flex-shrink: 0; }
.base-navbar__bottom { width: 100%; }
.base-navbar__bottom-inner { position: relative; z-index: 1; width: 100%; box-sizing: border-box; padding-bottom: 12px; }
</style>